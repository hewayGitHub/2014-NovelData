    ...
    for table_id in xrange(0, 256):
        ...
        for index in xrange(0, 10):
            ...
            sql = 'SELECT book_name, pen_name ' \
                  ...
                  'WHRER dir_id = {1}'.format(dir_id % 256, dir_id)
            cursor = conn.cursor()
            cursor.execute(sql)
            ...
            sql = 'SELECT chapter_title, raw_chapter_title ' \
                  'FROM novel_cluster_chapter_info{0} ' \
                  'WHERE dir_di = {1}'.format(dir_id % 256, dir_id)
            cursor = conn.cursor()
            cursor.execute(sql)
            for (chapter_title, raw_chapter_title, ) in cursor.fetchall():
                print('chapter_title: {0}, raw_chapter_title: {1}'.format(chapter_title, raw_chapter_title))
            cursor.close()
